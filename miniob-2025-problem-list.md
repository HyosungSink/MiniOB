...
本地评测：
- 测试功能：基础建表、建索引、插入、查询、删除后再插入，以及基础错误处理。
- Case 1:
  - Test case 语句:
```sql
SELECT * FROM basic_t WHERE id=1;
SELECT id, age, name FROM basic_t WHERE age=18;
```
  - 期望结果:
```text
1 | 18 | A | 1.5
...
- Case 2:
  - Test case 语句:
```sql
DELETE FROM basic_t WHERE id=2;
INSERT INTO basic_t VALUES(4, 30, 'd', 4.5);
SELECT id, score FROM basic_t;
```
  - 期望结果:
```text
SUCCESS
...
- Case 3:
  - Test case 语句:
```sql
CREATE TABLE basic_t(id int);
INSERT INTO basic_t VALUES(5);
SELECT missing_col FROM basic_t;
```
  - 期望结果:
```text
FAILURE
...
本地评测：
- 测试功能：带索引列和普通列的条件更新、无条件批量更新、更新后可见性和类型/对象错误处理。
- Case 1:
  - Test case 语句:
```sql
UPDATE update_t SET score=9.5 WHERE id=1;
SELECT id, score FROM update_t WHERE id=1;
```
  - 期望结果:
```text
SUCCESS
...
- Case 2:
  - Test case 语句:
```sql
UPDATE update_t SET age=30 WHERE age=20;
SELECT id, age FROM update_t;
```
  - 期望结果:
```text
SUCCESS
...
- Case 3:
  - Test case 语句:
```sql
UPDATE update_t SET tag='z';
SELECT id, tag FROM update_t;
```
  - 期望结果:
```text
SUCCESS
...
- Case 4:
  - Test case 语句:
```sql
UPDATE update_t SET missing_col=1 WHERE id=1;
UPDATE missing_update_t SET age=1 WHERE id=1;
UPDATE update_t SET age='bad' WHERE id=1;
```
  - 期望结果:
```text
FAILURE
...
本地评测：
- 测试功能：删除带索引表、删除后资源不可访问、同名表重建和重复删除错误。
- Case 1:
  - Test case 语句:
```sql
SELECT * FROM drop_t;
...
- Case 2:
  - Test case 语句:
```sql
CREATE TABLE drop_t(id int);
INSERT INTO drop_t VALUES(3);
SELECT * FROM drop_t;
```
  - 期望结果:
```text
...
本地评测：
- 测试功能：DATE 类型存储、1970 年前/2038 年后日期、闰年、日期比较、更新和非法日期校验。
- Case 1:
  - Test case 语句:
```sql
SELECT id, birthday FROM date_t;
SELECT id FROM date_t WHERE birthday='2020-02-29';
SELECT id, birthday FROM date_t WHERE birthday<'1970-01-01' OR birthday>'2038-01-19';
```
  - 期望结果:
```text
1 | 1969-12-31
...
本地评测：
- 测试功能：INNER JOIN、多表连接、显式 join 与逗号连接混用、多条件 join 和错误引用处理。
- Case 1:
  - Test case 语句:
```sql
SELECT a.id, a.name, b.score FROM join_a AS a INNER JOIN join_b AS b ON a.dept=b.dept;
```
  - 期望结果:
```text
1 | A | 100
...
- Case 2:
  - Test case 语句:
```sql
SELECT a.id, b.score, c.flag FROM join_a AS a INNER JOIN join_b AS b ON a.dept=b.dept, join_c AS c WHERE c.id=a.id;
```
  - 期望结果:
```text
1 | 100 | 1
...
- Case 3:
  - Test case 语句:
```sql
SELECT a.id, b.score FROM join_a a INNER JOIN join_b b ON a.dept=b.dept AND b.score>=100 WHERE a.id>=2;
```
  - 期望结果:
```text
2 | 200
...
- Case 4:
  - Test case 语句:
```sql
SELECT a.id FROM join_a AS a INNER JOIN missing_join AS m ON a.id=m.id;
SELECT a.missing_col FROM join_a AS a INNER JOIN join_b AS b ON a.dept=b.dept;
```
  - 期望结果:
```text
FAILURE
...
本地评测：
- 测试功能：投影表达式、WHERE 表达式、嵌套算术表达式、除零 NULL 语义和表达式错误处理。
- Case 1:
  - Test case 语句:
```sql
...
- Case 2:
  - Test case 语句:
```sql
SELECT id, a * b AS mul_value, a / b AS div_value FROM expr_t;
```
  - 期望结果:
```text
1 | 20 | 5
...
- Case 3:
  - Test case 语句:
```sql
SELECT id FROM expr_t WHERE 5 + b < a + 6;
...
本地评测：
- 测试功能：`length`、`round`、`date_format` 的正常输出、NULL 传播、非法格式符处理和参数数量校验。
- Case 1:
  - Test case 语句:
```sql
SELECT id, length(name), round(score), round(score, 1) FROM function_t;
```
  - 期望结果:
```text
1 | 1 | 1 | 1.2
...
本地评测：
- 测试功能：多列索引创建、组合条件查询、索引列更新、带索引条件删除和索引相关错误处理。
- Case 1:
  - Test case 语句:
```sql
CREATE INDEX idx_multi_ab ON multi_index_t(a, b);
SELECT id, c FROM multi_index_t WHERE a=10 AND b>=1;
```
  - 期望结果:
```text
SUCCESS
...
SELECT id, a, b FROM multi_index_t WHERE a=10;
```
  - 期望结果:
```text
SUCCESS
...
本地评测：
- 测试功能：单列唯一索引、多列唯一索引、插入/更新冲突、drop index 后约束解除和错误索引操作。
- Case 1:
  - Test case 语句:
```sql
INSERT INTO unique_t VALUES(3, 300, 2, 'a');
SELECT id, email, region, tag FROM unique_t;
```
  - 期望结果:
```text
SUCCESS
...
- Case 2:
  - Test case 语句:
```sql
INSERT INTO unique_t VALUES(4, 100, 3, 'c');
INSERT INTO unique_t VALUES(5, 500, 1, 'a');
UPDATE unique_t SET email=200 WHERE id=3;
```
  - 期望结果:
```text
FAILURE
...
- Case 3:
  - Test case 语句:
```sql
DROP INDEX idx_unique_email ON unique_t;
INSERT INTO unique_t VALUES(6, 100, 3, 'd');
SELECT id, email FROM unique_t;
```
  - 期望结果:
```text
SUCCESS
...
- Case 4:
  - Test case 语句:
```sql
DROP INDEX missing_unique_idx ON unique_t;
CREATE UNIQUE INDEX idx_unique_bad ON unique_t(missing_col);
```
  - 期望结果:
```text
FAILURE
...
本地评测：
- 测试功能：单列/多列 GROUP BY、`count/sum/avg/min/max`、NULL 聚合、无分组聚合和分组语义错误。
- Case 1:
  - Test case 语句:
```sql
SELECT dept, count(*), count(score), sum(score), avg(score), min(score), max(score) FROM group_t GROUP BY dept;
```
  - 期望结果:
```text
1 | 3 | 3 | 60 | 20 | 10 | 30
...
- Case 2:
  - Test case 语句:
```sql
SELECT dept, category, count(*), sum(score) FROM group_t GROUP BY dept, category;
```
  - 期望结果:
```text
1 | A | 2 | 30
...
本地评测：
- 测试功能：简单 IN/NOT IN 子查询、标量聚合子查询、聚合比较和子查询错误处理。
- Case 1:
  - Test case 语句:
```sql
...
本地评测：
- 测试功能：列别名、无 AS 别名、表达式/聚合别名、表别名、join 与子查询中的别名引用和别名错误。
- Case 1:
  - Test case 语句:
```sql
...
本地评测：
- 测试功能：NULL/NOT NULL 字段、`IS NULL`、`IS NOT NULL`、NULL 表达式、NULL 聚合计数、更新为 NULL 和约束错误。
- Case 1:
  - Test case 语句:
```sql
SELECT id FROM null_t WHERE age IS NULL;
SELECT id FROM null_t WHERE score IS NOT NULL;
```
  - 期望结果:
```text
2
...
- Case 2:
  - Test case 语句:
```sql
SELECT id, age + 1 AS age_plus FROM null_t WHERE id=2;
SELECT count(*), count(age), count(score) FROM null_t;
```
  - 期望结果:
```text
2 | NULL
...
- Case 3:
  - Test case 语句:
```sql
UPDATE null_t SET age=NULL WHERE id=1;
SELECT id, age FROM null_t;
```
  - 期望结果:
```text
SUCCESS
...
- Case 4:
  - Test case 语句:
```sql
INSERT INTO null_t VALUES(NULL, 1, 1.0, 'x');
SELECT id FROM null_t WHERE missing_col IS NULL;
```
  - 期望结果:
```text
FAILURE
...
本地评测：
- 测试功能：UNION 去重、UNION ALL 保留重复、多段 UNION、过滤/别名参与 UNION 和错误分支处理。
- Case 1:
  - Test case 语句:
```sql
SELECT id, tag FROM union_a UNION SELECT id, tag FROM union_b;
```
  - 期望结果:
```text
1 | A
...
- Case 2:
  - Test case 语句:
```sql
SELECT id, tag FROM union_a UNION ALL SELECT id, tag FROM union_b;
```
  - 期望结果:
```text
1 | A
...
- Case 3:
  - Test case 语句:
```sql
SELECT id FROM union_a UNION SELECT id FROM union_b UNION SELECT id FROM union_c;
SELECT id AS value_id FROM union_a WHERE id<=2 UNION SELECT id AS value_id FROM union_b WHERE id>=2;
SELECT tag FROM union_a WHERE id IN (SELECT id FROM union_b) UNION ALL SELECT tag FROM union_c;
```
  - 期望结果:
```text
1
...
- Case 4:
  - Test case 语句:
```sql
SELECT id FROM union_a UNION SELECT id, tag FROM union_b;
SELECT id FROM union_a UNION SELECT id FROM missing_union;
SELECT id FROM union_a UNION ALL SELECT unknown_col FROM union_b;
```
  - 期望结果:
```text
FAILURE
...
本地评测：
- 测试功能：单列/多列排序、升降序混合、NULL 排序、LIMIT、表达式排序和排序错误处理。
- Case 1:
  - Test case 语句:
```sql
...
本地评测：
- 测试功能：VECTOR 类型建表/插入、`vector_to_string`、`string_to_vector`、三种距离函数和非法向量/维度错误。
- Case 1:
  - Test case 语句:
```sql
SELECT id, vector_to_string(emb) AS emb_text FROM vector_basic_t WHERE id=1;
```
  - 期望结果:
```text
...
- Case 3:
  - Test case 语句:
```sql
INSERT INTO vector_basic_t VALUES(4, string_to_vector('[1, x, 3]'));
SELECT vector_to_string(string_to_vector('[1, 2]')) AS short_vector;
SELECT distance(string_to_vector('[1, 2]'), string_to_vector('[1, 2, 3]'), 'EUCLIDEAN') AS bad_distance;
```
  - 期望结果:
```text
FAILURE
...
本地评测：
- 测试功能：TEXT 字段插入、长文本/空串/NULL、长度计算、更新、等值过滤和基础错误处理。
- Case 1:
  - Test case 语句:
```sql
SELECT id, content FROM text_t WHERE id=1;
SELECT id, length(content) AS content_len FROM text_t WHERE id IN (2, 3, 4) ORDER BY id;
```
  - 期望结果:
```text
1 | HELLO MINIOB DATABASE
2 | 45
3 | 0
4 | NULL
```
- Case 2:
  - Test case 语句:
```sql
UPDATE text_t SET content='updated text' WHERE id=3;
SELECT content FROM text_t WHERE id=3;
SELECT id FROM text_t WHERE content='hello miniob database' OR content='';
```
  - 期望结果:
```text
SUCCESS
...
- Case 3:
  - Test case 语句:
```sql
INSERT INTO text_t VALUES(5);
SELECT missing_col FROM text_t;
```
  - 期望结果:
```text
FAILURE
...
本地评测：
- 测试功能：无索引精确向量检索、距离函数排序、LIMIT 最近邻、三种距离排序和向量错误处理。
- Case 1:
  - Test case 语句:
```sql
...
本地评测：
- 测试功能：ADD COLUMN、CHANGE COLUMN、DROP COLUMN、RENAME TABLE、索引列删除交互和 ALTER 错误处理。
- Case 1:
  - Test case 语句:
```sql
...
SELECT id, years FROM alter_base;
...
本地评测：
- 测试功能：MVCC 模式下 update 的事务内可见性、commit、rollback、多连接提交前后可见性和索引列更新。
- Case 1:
  - Test case 语句:
```sql
BEGIN;
...
本地评测：
- 测试功能：相关 EXISTS 子查询、按外层字段关联的聚合子查询、嵌套 IN、标量聚合子查询和复杂子查询错误处理。
- Case 1:
  - Test case 语句:
```sql
SELECT u.id, u.name FROM complex_user u WHERE EXISTS (SELECT 1 FROM complex_order o WHERE o.uid=u.id AND o.amount>150);
```
  - 期望结果:
```text
1 | A
...
- Case 2:
  - Test case 语句:
```sql
SELECT id, name FROM complex_user outer_u WHERE score>(SELECT avg(score) FROM complex_user inner_u WHERE inner_u.dept=outer_u.dept);
```
  - 期望结果:
```text
2 | B
...
- Case 3:
  - Test case 语句:
```sql
SELECT id FROM complex_user WHERE dept IN (SELECT dept FROM complex_user WHERE id IN (SELECT uid FROM complex_order WHERE amount>=300));
...
本地评测：
- 测试功能：简单可更新视图、视图 insert/update/delete、别名过滤视图、join 视图、聚合视图和视图错误处理。
- Case 1:
  - Test case 语句:
```sql
...
本地评测：
- 测试功能：jieba 分词、全文索引创建、MATCH AGAINST 查询、BM25 排序、索引维护和全文索引错误处理。
- Case 1:
  - Test case 语句:
```sql
SELECT tokenize('<CJK_QUERY_INFORMATION_SCHEMA>', 'jieba') AS text_tokens;
SELECT tokenize('', 'jieba') AS text_tokens;
```
  - 期望结果:
```text
["information", "schema", "SCHEMATA", "<CJK_TOKEN>", "<CJK_TOKEN>", "<CJK_TOKEN>"]
[]
```
- Case 2:
  - Test case 语句:
...
SELECT id, match(content) against('<CJK_QUERY_DATABASE>') AS score FROM fti_docs WHERE match(content) against('<CJK_QUERY_DATABASE>') > 0 ORDER BY match(content) against('<CJK_QUERY_DATABASE>') DESC, id ASC;
...
- Case 3:
  - Test case 语句:
```sql
INSERT INTO fti_docs VALUES(6, '<CJK_DOC_DATABASE_DATABASE_INDEX>');
...
本地评测：
- 测试功能：多张 20 列表的笛卡尔积排序、多字段排序、LIMIT 截断、长排序键前缀和排序错误处理。
- Case 1:
  - Test case 语句:
```sql
SELECT a1, b1, c1, d1 FROM big_order_a, big_order_b, big_order_c, big_order_d ORDER BY a1 ASC, b1 DESC, c1 ASC, d1 DESC LIMIT 10;
```
  - 期望结果:
```text
1 | 4 | 0 | 7
...
- Case 2:
  - Test case 语句:
```sql
SELECT a1, a2, a3 FROM big_order_a ORDER BY a1 ASC, a2 ASC, a3 DESC;
```
  - 期望结果:
```text
1 | 3 | 2
...
- Case 3:
  - Test case 语句:
```sql
SELECT b1, c1, d1 FROM big_order_b, big_order_c, big_order_d ORDER BY b1 ASC, b2 DESC, c1 ASC, c2 DESC, d1 ASC, d2 DESC LIMIT 20;
SELECT a1, b1, c1, d1 FROM big_order_a, big_order_b, big_order_c, big_order_d ORDER BY a1, a2, a3, a4, a5, b1, b2, b3, b4, b5, c1, c2, c3, c4, c5, d1, d2, d3, d4, d5 LIMIT 15;
```
  - 期望结果:
```text
LIMIT 20
...
- Case 4:
  - Test case 语句:
```sql
SELECT a1 FROM big_order_a ORDER BY missing_col;
SELECT a1 FROM big_order_a ORDER BY;
```
  - 期望结果:
```text
FAILURE
