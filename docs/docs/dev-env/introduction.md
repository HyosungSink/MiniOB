...
首先，大家需要清楚 MiniOB 当前只能在 Linux/MacOS 操作系统上编译。因此，如果是使用 Windows 操作系统的同学，需要首先准备 Linux 的虚拟环境（例如 WSL2 或虚拟机）。
...
## Windows 开发环境
对于使用 Windows 操作系统的同学，需要先准备 Linux 的虚拟环境（例如 WSL2 或虚拟机），之后才能在 Linux 的虚拟环境上开发 MiniOB。
## 远程开发环境
对于有远程开发环境（例如云服务器等）的同学不受本地操作系统的限制，可以参考本地 VSCode 调试文档中的远程调试配置思路。
## 调试（Debug）介绍
